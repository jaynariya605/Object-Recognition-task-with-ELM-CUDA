...
Mat	= (double *)malloc(3060	* 5048	* sizeof(double));
Res	= (double *)malloc(1	* 3060	* sizeof(double));
Mat_test	= (double *)malloc(6084	* 5048	* sizeof(double));
Res_test	= (double *)malloc(1	* 6084	* sizeof(double));

cudaMalloc((void**)&training, sizeof(double)*3060*5048) ;
cudaMalloc((void**)&t, sizeof(double)*1*3060) ;
cudaMalloc((void**)&testing, sizeof(double)*6084*5048) ;
cudaMalloc((void**)&t_test, sizeof(double)*1*6084) ;


ReadCSV(Mat,"new_F_train.csv");
ReadCSV(Res,"train_labels.csv");
ReadCSV(Mat_test,"new_F_test.csv");
ReadCSV(Res_test,"test_labels.csv");


cudaMemcpy(training	, Mat		, 3060	 * 5048* sizeof(double), cudaMemcpyHostToDevice);
cudaMemcpy(t	, Res		, 3060	 * 1* sizeof(double), cudaMemcpyHostToDevice);
cudaMemcpy(testing	, Mat_test		, 6084	 * 5048* sizeof(double), cudaMemcpyHostToDevice);
cudaMemcpy(t_test	, Res_test		, 6084	 * 1* sizeof(double), cudaMemcpyHostToDevice);


ELM(training,t,testing,t_test,10000,3060,6084,5048);
